...
`spawn` 与 `spawnTerminal` 每启动一个子进程就发出一次 `subprocess/launched`,发出者是这条缝隙而不是各个实现,因此一个部署无需每个 spawner 各自上报,就能回答"它运行过什么"。实现覆写的是 `spawnProcess` 与 `spawnTerminalSession`;自报的那一半属于这条缝隙。

记录指名可执行文件、工作目录、pid,以及子进程是否拥有一个终端。它不指名参数,也不指名环境。一个 spawner 的 argv 里装的是调用方放进去的任何东西——一段模型提示词、一个作为命令行开关传入的凭据——而环境正是凭据所在之处;一条关于启动的记录,不应该成为两者外泄的途径。需要更多信息的消费方,知道的本就比这条缝隙多。

pid 为 `-1` 表示这次启动失败了。无论如何都会返回一个句柄,因此这条记录是在报告一次没有发生的启动,而不是干脆不存在。

归属不在这里。属于哪个租户、哪次运行、哪个智能体——这条缝隙对它们一无所知,由拥有这些信息的消费方补上。
