...
import { apply, checkDeepSeekCredential } from "../src/index.js";
const secret = new TextEncoder().encode('test-key');
describe('DeepSeek credential check', () => {
    it.each([[200, true, undefined], [401, false, 'invalid-credential'], [403, false, 'invalid-credential'], [429, false, 'provider-unavailable'], [500, false, 'provider-unavailable']])('maps HTTP %s without exposing provider details', async (status, valid, reason) => {
    ...
    it('contains transport failures and malformed or empty credentials', async () => {
        const unavailable = await checkDeepSeekCredential(secret, {
            ...
            fetch: vi.fn(async () => { throw new Error('endpoint and body must stay private'); }),
        });
    ...
    it('registers only the deepseek-api provider in the existing registry', async () => {
        ...
        apply(ctx, { baseURL: 'https://provider.invalid', timeoutMs: 10 });
