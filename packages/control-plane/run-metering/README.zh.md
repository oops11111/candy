...
`RunScheduler.meter` 把一次运行的多次调用排成一列，于是每一次读到的剩余额度，都是它前面那一次已经被记过账之后的。这一列是按运行来的，因此两个租户永不彼此等待；而调用本就顺序发生的运行——agent 循环就是——也从不等待，因为轮到它下一次调用时这一列已经空了。半途放弃一条流的消费方同样会退出这一列；否则该运行之后的每一次调用都会等在一条没人排空的流上。
