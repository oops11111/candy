...
### 为什么截止时刻是被等待的，而不是在块之间被检查

一个接受了请求随后就安静下来的提供方不产出任何块，因此块之间的检查无从下手：一次被允许 50 毫秒挂钟时间的运行，实测在整整一秒之后仍在等待，除了它的租约之外没有任何东西界定它。每一次读取都与运行剩下的时间赛跑，于是沉默的失败与话多的失败终结在同一个维度上。

时钟不仅被用来赛跑，也在每次读取之前被读一次。一个已经越过截止时刻的调用方时钟不会拿到任何定时器，因为 `setTimeout` 把非正的延迟当作一个 tick——那会从一个时间已经耗尽的源里再读一个块。
...
一次拒绝就是消费方所能看到的全部——一个终止的 `error` finish——而它对任何观察这套部署的人什么也没说。可选的 `refused` 端口正是保有审计踪迹的调用方记录这次调用的地方,而它在该块被产出之前被等待,因此读取踪迹的运维人员绝不会落后于一个已经据此行动的消费方。

该端口的实现自行了结自己的失败。这里的一次拒绝会让流缺少本缝隙所承诺的那唯一一个终止块,而那比一次未被记录的拒绝更糟。手工计量的调用方什么也不传,于是这些拒绝不会被记录。

### 为什么一次运行的多次调用要彼此等待

计量器在调用提供方之前读一次运行还能花什么，并在调用结束时记一次账。因此两次重叠的调用都会针对一份谁都还没被记过账的剩余额度起步：一次只够跑一次假适配器调用的运行花掉了两次调用的量——42 的额度记下了 84 个 token。这些维度界定的是运行，不是调用，因此它们只有在多次调用不观察到同一份剩余额度时才成立。

`RunScheduler.meter` 把一次运行的多次调用排成一列，于是每一次读到的剩余额度，都是它前面那一次已经被记过账之后的。这一列是按运行来的，因此两个租户永不彼此等待；而调用本就顺序发生的运行——agent 循环就是——也从不等待，因为轮到它下一次调用时这一列已经空了。半途放弃一条流的消费方同样会退出这一列；否则该运行之后的每一次调用都会等在一条没人排空的流上。它是在 `finally` 里让出这一列的,因为关闭也可能失败——一次被取消的调用关闭的是一个本身正在失败的源,而一个拆解时抛出的提供方,否则会把它那次运行搁浅到其生命结束。
...
- **被切断的源是被通知关闭，而不是被守着关完** —— 在截止时刻这条路径上，关闭只是被发起而不被等待，因为一个卡在同样沉默里的源同样不会回应它。因此提供方进程由拥有它的那一方收割，而不是在本调用返回时就已收割完毕。
- **token 每次调用只计一次费** —— 一条流至多报告一次用量，因此过长的单次回复只有在它结束时才被量出来。挂钟切断界定的是这一次调用；token 切断界定的是下一次。
- **并发的调用各自读到同一份剩余额度** —— 针对同一次运行计量的两条流，都是针对谁都还没记过账的那份额度起步的。为一次运行的多次调用排序属于调用方：[`dsh-run-scheduler`](../run-scheduler/README.zh.md) 按运行把它们排成一列，而手工计量的调用方要自己承担这件事。
