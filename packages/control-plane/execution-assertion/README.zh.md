...
版本前缀与载荷一起处在 MAC 之内。它决定了载荷如何被读取,而只覆盖载荷的签名在任何前缀之下都能通过校验——因此一旦出现 `v2` 声明集,一个被改标为 `v1` 的 `v2` 令牌仍会通过校验,随后被 `v1` 的读取者解码。那恰恰是版本前缀本该阻止的重新解读,而只有把版本也签进去,这个前缀才有意义。分隔符就是令牌本就使用的那一个,而且它不可能出现在任一段之内,因为两段都是 base64url。
