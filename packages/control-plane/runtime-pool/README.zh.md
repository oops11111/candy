...
- **不检查池基目录自身的权限**——一个另一个本地账户可写的基目录,会让该账户在本包见到之前就创建或替换掉某个池根目录,而此后本调用设置的任何模式都无法从中恢复。把基目录准备成私有的是部署的职责,而在 Windows 上是 `dsh-sandbox-windows-acl` 的职责,因为 POSIX 模式位并不描述该平台的访问控制。
