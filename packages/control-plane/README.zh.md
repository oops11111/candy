...
| [`device-api`](device-api/README.zh.md) | 挂在管理信封上的三个租户设备操作,以及主机仅凭配对码、不带会话完成的配对兑换 |
| [`device-binding`](device-binding/README.zh.md) | 主机自身关于「服务哪个部署、扮演哪台设备」的记录,天然唯一,只能释放而不能替换 |
...
| [`deepseek-credential-check`](deepseek-credential-check/README.zh.md) | 为 `deepseek-api` 注册脱敏的认证 `/models` 探测 |
