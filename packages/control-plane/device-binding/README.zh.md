...
连接不在这里。抵达服务器、察觉链路断开、退避与重连,都属于继承而来的传输层,本包对此毫无增补。它回答的是该抵达哪一台服务器,以及以谁的身份。
...
- **没有任何东西用它去连接** —— 本服务回答该抵达哪台服务器、以谁的身份。本仓库中没有任何传输读取它,也没有任何主机出示它所持有的令牌。
- **这里察觉不到设备已被撤销** —— 服务端会拒绝已撤销设备的下一次运行,而本主机会一直保留绑定,直到运维释放它。要知道一条绑定已经失效就得去问服务器,而目前没有任何东西这样做。
