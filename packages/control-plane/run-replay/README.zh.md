...
- **一个进程，而不是一个部署** —— 记录在实例存活期间存于内存。共享同一控制平面的两个运行时进程会各自准入同一个令牌的一份副本，而一次重启会遗忘每一个尚未过期的 nonce。运行多于一个进程的部署需要一个持久化存储，而本包的契约正是那个存储必须满足的东西。而仓库里还没有任何东西能满足它：那个不可分割的步骤需要一种「键已存在就失败」的写入，而 `dsh-storage` 提供的 `putRecord` 是无条件 upsert，读取则由打开时加载的快照作答。一个持久化存储要等那里出现一个条件写入原语。
