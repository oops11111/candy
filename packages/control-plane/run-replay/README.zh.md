...
本包是该端口的进程内实现，也是对任何一种实现所欠下的义务的陈述。三件事决定一个重放存储能否工作，而每一件都容易做错：决定必须是一个不可分割的步骤、保留期必须由断言而不是由定时器界定，以及记录必须按租户分区。Candy 的生产组合在 [`dsh-control-plane-store`](../control-plane-store/README.zh.md) 中持久实现同一契约。
...
- **这个类只覆盖一个进程，而不是整个部署** —— `RunReplayStore` 仍是供库消费方和测试使用的小型同步实现。Candy 的调度器不再使用它:`dsh-control-plane-store` 通过 SQLite compare/exchange 持久保存同一个租户加 nonce 的决定,因此多个运行时进程和重启会共享该记录。
