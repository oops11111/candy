...
A refused *call* is filed the same way, under `event: 'refused'`, with the failure code as its outcome. Metering decisions use `action: 'meter'`; `recordRouteRefusal()` records a final tenant route-policy decision as `action: 'route'` before the denial is reported. Each managed call also records the provider/model pair left by the Harness routing waterfall as `routed/select/ok` before its metered stream reaches the adapter.

A managed tool remains a Harness tool. This service only observes its final authorization decision, so cancellation continues to travel on the caller's signal through the existing tool runtime. A started body is allowed to finish its cleanup before `ABORTED` is returned and the single terminal `tools/result` event is published. The Loader composition test holds a body open after cancellation and proves both the result and its event stay pending; this is evidence that Candy's awaited audit observer has not changed the inherited lifecycle, not a second Candy executor or event transport.
...
- **Ended-session caching is bounded; ownership is durable** — `endedSessionMemory` bounds the cache. The control-plane store retains ownership after settlement, so cache eviction and restart never allow an owned session to call without an open run. A queued stream closed before its turn never starts its provider source.
...
- **The trail omits sensitive tool detail** — managed calls record a tool name and final `allowed` or `denied` authorization outcome, but never arguments, denial reasons, or results. Calls not owned by one open Candy run remain ordinary Harness activity and are not attributed to a tenant.
