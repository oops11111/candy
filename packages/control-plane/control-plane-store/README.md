...
`beginOAuthAttempt` creates independent 256-bit state, PKCE verifier, and OIDC nonce values, stores the verifier and nonce server-side, and returns the state, S256 challenge, and nonce needed by the authorization request. `consumeOAuthAttempt` atomically removes a matching transaction before returning its issuer, redirect URI, verifier, and nonce; a wrong, expired, concurrent, replayed, or pre-nonce callback receives nothing. The record survives a restart so a callback may land on another control-plane process sharing SQLite.
