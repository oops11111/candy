...
本服务持有它们：可撤销的 OAuth 用户会话、提供方账户及其密封凭据、每个租户的额度与精确模型路由策略、设备签发的工作区授权、每次存活运行的记录，以及每个租户运行活动的踪迹；它们都位于一个使用 SQLite 后端的[存储域](../../../docs/subsystems/storage.zh.md)中。重启之后它们仍然存在。

它不是账本。`RunLedger` 仍然是记账权威,回答一次运行还能花什么;住在这里的是能挺过重启的那条记录,以及让一次被打断的结算恰好完成一次的那两个标记。每次运行之前先写入会话归属，结算之后仍然保留；`isManagedSession` 可在没有存活运行时按所属运行时识别这些会话。存储域版本 11 拒绝旧记录；升级已有部署需要单独验证数据转换。终态审计记录保留最终用量，路由调用记录保留选定的 provider/model 组合，工具记录则保留最终授权结果，但不包含参数或理由。归属记录不会自动过期。
