---
description: "为 Candy 控制平面持久保存提供方账户、租户额度与路由策略。"
...
本服务持有它们：可撤销的 OAuth 用户会话、提供方账户及其密封凭据、每个租户的额度与精确模型路由策略、租户配对的设备与配对它们的配对码、设备签发的工作区授权、每次存活运行的记录，以及每个租户运行活动的踪迹；它们都位于一个使用 SQLite 后端的[存储域](../../../docs/subsystems/storage.zh.md)中。重启之后它们仍然存在。

它不是账本。`RunLedger` 仍然是记账权威,回答一次运行还能花什么;住在这里的是能挺过重启的那条记录,以及让一次被打断的结算恰好完成一次的那两个标记。每次运行之前先写入会话归属，结算之后仍然保留；`isManagedSession` 可在没有存活运行时按所属运行时识别这些会话。存储域版本 12 拒绝旧记录；升级已有部署需要单独验证数据转换。终态审计记录保留最终用量，路由调用记录保留选定的 provider/model 组合，工具记录则保留最终授权结果，但不包含参数或理由。归属记录不会自动过期。
...
该调用会替换完整白名单。精确 id 区分大小写；空白字段或重复组合会被拒绝。空列表会持久保存一份显式全拒绝策略，而 `tenantModelRoutes(userId) === undefined` 表示尚未设置策略。[`dsh-tenant-route-policy`](../tenant-route-policy/README.zh.md) 会拒绝这两种状态。
...
域会把每一次写入串行化,却不会把决定「写什么」的那次读取也串行化。两个调用方在任何一方写入之前都读到同一条记录,于是各自基于同一个值计算,而第二次写入会把第一次丢掉 —— 一笔丢失的记账,或者一条丢失的审计记录。把读与它所喂养的那次写放在同一条链上,才让「向踪迹追加」与「对额度记账」可以被并发调用。

这条链是整个存储一条,而不是每条记录一条。这些是结算、审计追加与授予额度变更,不是热路径,而一条链无需任何本身也可能出错的簿记就已经正确。
...
一条有界且被整体重写的踪迹,同样可以被任何能让某个事件反复发生的人抹掉。在保留上限为四时,八次被拒绝的调用只留下了那些拒绝本身,而运维人员用来调查它们的凭据记录与启动记录都没了。因此,一条除了发生时刻之外与最新那条在每个字段上都相同的记录,会被折进那一条——变成一个 `count` 和一个更晚的 `at`——而不是把历史一条一条挤出去。它并不增加踪迹本就能区分的任何东西,而它本会挤掉的,恰恰是让这次反复值得一读的那些内容。
...
一份执行断言按 id 指名一份工作区授权,而记录才是它背后的权限。一条被删除的记录读起来像是一份从未签发过的授权,而这与一份被撤回的授权是不同的事实 —— 而且为它被拒绝的运行应当说出 `revoked`,因为这正是排查这次拒绝的运维人员所需要的。[`dsh-workspace-grant`](../workspace-grant/README.zh.md) 只读 `revokedAt`,不读别的,来判定是哪一种。

### 为什么配对码是被认领的,而不是读出来再写回去

配对码只能用一次,而兑换它的两台主机都会读到它未兑换。因此 `claimPairingCode` 在一次 compare/exchange 中同时决定兑换与有效期——与 `spendNonce` 使用的机制相同,理由也相同:只有交换能把两者区分开,而且它必须跨进程和跨重启成立,而不仅仅是在本进程的队列之内。

终态配对码的保留清理由注册表从单一租户中选出记录后调用 `deletePairingCode`。消费和过期都不会逆转，因此该删除不需要兑换所用的 compare/exchange；尚未终态的配对码绝不会被选中。

### 为什么令牌查找要重读介质

`entriesCurrent` 会从介质发现候选项，即使它是在本进程启动后由另一进程创建；随后 `getCurrent` 确认该候选仍出示相同摘要。被另一进程撤销或替换的设备绝不能依据旧本地快照完成认证。同样的显式介质读取也服务于设备 id、配对码认领以及租户设备／配对码列表，因此请求路由不会决定调用方看见哪份设备状态。

### 为什么运行记录会指名它的账户

子运行继承其父运行授权的一个子集,而租户与账户是运行时能够裁定的那两项:父运行各自只持有一个。`findRun` 正是 `dsh-run-admission` 用来核对子运行所声明身份的东西,而这条记录就是父运行的身份被写下来的地方。

### 为什么运行记录要写下它的设备、workspace 授权与对话

一次执行断言的设备、workspace 授权与对话这几项 claim,此前只在验证那一个 token 期间存在;在此之前,没有任何东西会把它们留存到准入之后。`RunScheduler.startChildRun` 需要从一次已经开启的运行铸造出一个委派子运行自己的断言,而一份铸造出来的断言不能陈述一个运行时早已不再持有的事实——因此一条运行记录携带这三个字段,好让子运行的 claims 能从其父运行那里把它们复制过来。
...
- **恢复只修复一种损坏形态,而不是每一种** —— 一条指名了存储并不持有的父运行的记录,会被结算到它自己的租户头上并被清除,因为恢复本来就会结算它所恢复的每一个根。它没有指名的损坏——一条通不过 schema 的记录、一份已经消失的租户额度——仍然会让启动失败,而那些没有修复路径。
- **一个 audience 一个运行时** —— `runsOf` 按运行时戳记分区,因此共享同一 audience 的两个进程会互相恢复对方的记录。断言本来就绑定在 audience 上,因此这是一条部署规则,而不是这里所做的检查。
- **没有任何东西清理用尽的配对码** —— 已兑换和已过期的记录留在 `pairing_codes` 中,不像 `spent_nonces` 有 `evictNonces` 清扫。持续签发配对码的部署会让该表不断增长。
- **设备与配对可见性是拉取式的** —— 它们的公开存储读取会从介质显式刷新已知记录或相关整张表，因此另一存活进程的配对与撤销会到达下一次读取。`storage-domain` 仍不发送跨进程变更事件；消费方若把某次操作返回的记录缓存到该操作之外，仍会再次造成陈旧缓存。
...
- **读—改—写在整个存储范围内串行** —— 因此一个缓慢的介质会把某个租户的一次记账排在另一个租户的一次审计追加之后。替代方案是按记录分链,而目前没有任何东西需要它。
- **踪迹有界,而且整份重写** —— 一个主体的记录住在同一份文档里,因此每一次追加都会重写那份文档,而超出上限的记录是被丢弃而不是被归档。它适合一个部署量级上的近期活动窗口,不适合一个目录服务量级上的审计档案库。
- **踪迹记录的是控制平面观察到的东西** —— 调度、保险库、委派、路由、工具授权、提供方进程启动、用量与终止状态。工具参数、拒绝理由、结果，以及受管调用之外的进程启动会被有意省略。
