...
快照只把扫描收窄到一个候选;答案来自 `getCurrent`。被另一个进程撤销的设备在本进程的快照里仍然成立,而依据快照完成认证,等于放行了租户已经撤回的绑定。在同一 id 下被替换的记录不再出示那个摘要,因此过期的快照解析到的是无人,而不是它的后继者。
...
- **设备与配对码的读取是进程本地快照,令牌查找除外** —— `findDevice` 与两个列举操作依据本进程的快照作答,因此另一个进程配对或撤销的设备,要等到本进程重启才可见。`findDeviceByTokenDigest` 与 `claimPairingCode` 会触及介质,因为放行已撤回的绑定和让一个配对码配对两次,正是绝不能发生的两种失败。
