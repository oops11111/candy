...
它不是账本。`RunLedger` 仍然是记账权威,回答一次运行还能花什么;住在这里的是能挺过重启的那条记录,以及让一次被打断的结算恰好完成一次的那两个标记。每次运行之前先写入会话归属，结算之后仍然保留；`isManagedSession` 可在没有存活运行时按所属运行时识别这些会话。存储域版本 10 拒绝旧记录；升级已有部署需要单独验证数据转换。终态审计记录保留最终用量，路由调用记录保留选定的 provider/model 组合。归属记录不会自动过期。
