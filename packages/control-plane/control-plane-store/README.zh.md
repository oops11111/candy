...
本服务持有它们:提供方账户连同它们密封的凭据、每个租户的额度、每一次存活运行的一条记录,以及一条记录每个租户的运行做了什么的踪迹 —— 从开启一次运行的那次尝试,到结束它的那次结算,都放在一个走 SQLite 后端的[存储域](../../../docs/subsystems/storage.zh.md)里。重启之后它们还在 —— 这正是重点。
