...
拨款发生在准入之后,因此账本能够拒绝的时候,身份已经是已知的。一次只报告理由的拒绝,会说某次运行被拒了却不说是谁的 —— 这正是 [`dsh-run-admission`](../run-admission/README.zh.md) 已经为它自己的每一个步骤修好的那个缺陷,也是审计踪迹最无从绕开的那一个。因此这次拒绝携带已验证的声明。
...
- **在这里账本是内存里的** —— 本包不持有任何存储,因此回滚在一个进程之内把占用还回去,而无法找回一次崩溃所搁浅的占用。让记录变成持久的是 [`dsh-run-scheduler`](../run-scheduler/README.zh.md)。
