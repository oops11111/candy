...
被准入的运行携带已校验的声明、已打开的凭据、池键、池的目录,以及保险库对该次读取的审计记录。拒绝会指明产生它的阶段——`assertion`、`replay` 或 `credential`——因此运维人员可以区分伪造令牌与已吊销账户,而调用方不会因此得到任何可用于重试的信息。
...
- **审计记录只被返回,不被持久化**——`AdmittedRun` 携带保险库对该次读取的记录,而边界页面所要求的按租户分区存储由调用方拥有。拒绝时不返回任何审计记录,因此需要记录拒绝的调用方,应自行记录该 rejection。
