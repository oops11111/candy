...
当消费方必须从持久介质重新读取单个值、而不能信任打开时快照时，KV 单元可提供 `readRecord`。无法提供该保证的后端会省略它。
