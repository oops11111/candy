...
CLI 的 `--input-format stream-json` 看上去正是把一段对话交给它的那条通道。一次录制的运行依次喂入一条 user 消息、一条 assistant 消息和第二条 user 消息 —— 即 [`injected-history.jsonl`](../claude-cli-protocol/tests/fixtures/injected-history.jsonl) —— 确定了它实际做的事：两个会话、两个 `result` 帧、每条 user 消息各计一次费的回合，而那条 assistant 消息被接收后直接丢弃，没有任何一帧报告它。这条输入上没有任何东西能作为此前的助手内容到达模型。

因此把历史摊平到它上面不只是会丢失历史，而是会回答错的问题。翻译器在第一个终止帧上就已定案，于是调用方收到的会是对话*第一条*消息的回复，而对最后一条 —— 真正被问的那条 —— 的回复在付过费之后被丢弃。剩下的那条路是把对话渲染成提示词文本，那意味着发明一种本仓库没有依据的转录格式；一旦弄错就会悄悄降低模型输出质量，而 harness 中没有任何地方会显示出来。这个决定要等到出现真正需要它的消费者。
...
只有诊断文本会被改写。模型输出是租户自己的内容，悄悄编辑它会毁掉一个合法的回答 —— 比如一个关于密钥长什么样的回答。这次替换本身是 [`dsh-llm`](../llm/README.zh.md) 的 `redactChunkApiKey`,因此这条缝隙上的每一个适配器都用同样的方式移除凭据。

### 为什么 stderr 是被收集而不是被继承

脱敏只能触及这个适配器所携带的文本。`stderr: 'inherit'` 把父进程自己的描述符交给子进程,于是 CLI 直接写到宿主的流上,而这个适配器一个字节也看不到——而 CLI 写在那里的正是引用了失败请求的诊断,那个请求携带着租户的密钥。一次认证失败的运行会把注入的密钥明文写到运维人员的控制台上,而同样的文本从 stdout 到达时却被脱敏了。

这个流改为在 `maxStderrBytes` 之下被收集,并且不配置溢出文件:一个租户的诊断留在该运行自己的内存里,而不是共享的临时目录里。溢出时保留尾部,这与 stdout 上限相反,而理由相同——诊断以哪里失败了结尾,而协议流根本无法承受中间的缺口。

尾部不会被丢弃。一次进程死掉却没有终止帧的运行,没有任何别的关于自己的记述,因此尾部会并入那次失败的消息,并经由与其他每个块相同的脱敏离开。正常结束、被取消或触发 stdout 上限的运行已经说明了原因,其 stderr 会被丢弃。
...
- **成功运行的诊断会被丢弃** —— stderr 之所以被收集,是为了能把其中的凭据脱敏,而尾部只在一次没有终止帧就死掉的运行的失败里露面。曾经在宿主自己的流上看着 CLI 警告滚过去的运维人员,对一次已完成的运行不再看得到它们。把它们送往日志器而不只是送进一次失败,要等到出现真正会读取它的消费者。
