...
- **Guard a route before provider work** — `ctx.llm.guard()` registers a synchronous, monotonic authorization check before session-aware call preparation and again after waterfall routing, immediately before dispatch.
...
- **Route guards are early, final and monotonic** — session-aware preparation checks before adapter preflight, dispatch checks the pair left by waterfall routing, and any one refusal ends the call.
