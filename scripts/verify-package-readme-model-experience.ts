...
const NO_MODEL_EXPERIENCE_SECTION: Readonly<Record<string, string>> = {
  ...
  'packages/control-plane/device-registry': 'The package only decides which tenant a device is bound to; nothing it produces reaches a model request.',
  ...
  'packages/control-plane/device-binding': 'The package only records which deployment a host serves; nothing it produces reaches a model request.',
  ...
  'packages/control-plane/tenant-route-policy': 'The package only authorizes or refuses an already-assembled route; it adds no model-visible content to an allowed request, and a refused request reaches no model.',
...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/bundle/candy-app': { kind: 'indirect', reason: 'A patch layer over dsh-web-app; every model-facing effect belongs to a composed package, each of which documents its own.' },
  'packages/bundle/candy-host': { kind: 'none', reason: 'The one-shot device-management profile never starts an Agent or creates a provider request.' },
