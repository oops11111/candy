...
灰度实例绝不能与线上服务共用 `CANDY_DATABASE_PATH`，原因有两个。schema 是其一：灰度按定义就是你尚未信任其 schema 的那个版本。其二是，运行中的进程按它打开时的视图做认证，因此**在一个进程里退出登录不会传到另一个已经在运行的进程**——那个会话在那里一直有效，直到该进程重启。断言 nonce 是例外，它在两个进程之间恰好只被花掉一次，因为那次写入落到介质上；会话不是。给每个进程各自的数据库，这个问题就不存在。
