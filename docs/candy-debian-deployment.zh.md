...
灰度实例绝不能与线上服务共用 `CANDY_DATABASE_PATH`，因为灰度按定义就是你尚未信任其 schema 的那个版本。浏览器认证现在会从 SQLite 重新读取匹配的会话，因此一个进程退出登录后，另一个已运行进程会在下一次请求时拒绝该会话；断言 nonce 同样保持跨进程单次使用。这不代表混合版本访问已经安全：每个灰度实例仍须使用自己的数据库与运行时 audience。
