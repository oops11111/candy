...
Management endpoints must derive the acting user, tenant, and administrator role from a trusted OAuth-backed server session. Possession of a Harness Host access token does not establish any of those identities. The account and device endpoints are mounted on that session authority; route-policy and workspace-grant mutation endpoints remain unavailable until they are built on it too. One route is deliberately outside it: a host exchanging a pairing code has no session, and its request is authorized by the code alone and never receives an acting user.
