...
- [x] 为用户、设备、提供方账户、工作区授权、对话、会话、运行和子运行定义稳定标识符及 schema([`dsh-control-plane`](../../implemented/architecture/2026-09-02-candy-control-plane-identifiers.zh.md))。这套词汇还区分可撤销的 Candy `UserSessionId`、由外部证明的 `OAuthIdentity` issuer/subject 二元组，以及 Candy 分配的成员或管理员角色。`dsh-control-plane-store` 现在持久保存该会话，仅记录其 256 位 bearer 的摘要；每次认证都从记录推导身份与角色，并跨重启拒绝未知、过期或已撤销的 bearer。OAuth 回调验证以及 HTTP cookie/CSRF 传输仍未构建。
