...
跨进程发现现在与这项承诺一致（[只有一个进程看到的撤销](../../implemented/architecture/2026-09-12-a-revocation-only-one-process-saw.zh.md)）。最初的设备实现从每个进程启动时的快照读取 id 与配对码，因此在另一进程启动后签发的配对码或撤销的设备对它不可见。安全读取现在从持久介质重新读取已知键；当无法预先知道键时，令牌查询与租户设备／配对码列表会显式刷新整张表。这是拉取式可见性，不是第二套变更通知协议。
...
服务现在有了随发行提供的进程入口（[没有命令的配对码](../../implemented/architecture/2026-09-13-a-code-with-no-command.zh.md)）：独立的 `candy-host` profile 接受 `pair --server --code`，用 `status` 报告不含秘密的绑定，并且只在明确执行 `release` 时移除它。它不组合 Agent、Web 服务器、工具、Shell、Gateway 或重连循环；意外网络失败也不会把可能包含一次性配对码的请求转换成字符串后报告。

继承的设置面板现在有了租户自有的“设备”页。它签发一次性配对码，只在页面的易失状态中保留明文；列出不含令牌的设备与配对元数据，并通过现有已认证 API 撤销设备。这个页面是另一个 `settings.section`，不是第二套 Candy Web/手机端；Host 通过一次性的 `candy-host` profile 消费配对码。

仍然缺失的是传输对这项认证的使用。没有传输在建立或恢复连接时读取并出示绑定。继承而来的传输没有可以指向的远程主机这一概念：它的 Gateway 是浏览器到 Host 的路径，因此把令牌加在那里会认证错误的端点，也仍然无法让 Debian 调用 Windows 能力。租户可配对的设备数量仍没有上限。已消费和已过期的配对码记录现在按租户隔离，并在该租户列出设备或再次签发配对码时，按可配置保留窗口进行按需清扫；不活跃租户若要求按绝对时间删除，仍需外部维护触发器。
