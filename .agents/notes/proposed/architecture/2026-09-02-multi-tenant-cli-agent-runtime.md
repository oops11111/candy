...
- [ ] Implement the Claude CLI adapter with isolated home, non-interactive input, structured output parsing, cancellation, and process-tree cleanup. The direct-parser decision is settled: Candy parses `--output-format stream-json` itself rather than reusing the Agent SDK path `dsh-subagent-claude-code` takes, because that SDK supplies an agent loop where this seam needs a model call. The protocol half is built and measured against the real CLI ([`dsh-claude-cli-protocol`](../../implemented/architecture/2026-09-03-claude-cli-stream-protocol.md)): frame decoding, `StreamChunk` translation, and the argument vector and environment that confine a run to one tenant's credential, with two recorded runs as fixtures. What remains is the `LlmAdapter` itself — spawning through `ctx.subprocess`, projecting a harness request onto the CLI's single positional prompt, honoring `options.signal`, and process-tree cleanup.
