...
- [ ] Implement the Claude CLI adapter with isolated home, non-interactive input, structured output parsing, cancellation, and process-tree cleanup. Both inherited `LlmAdapter`s are HTTP-backed, so none drives a CLI. A working Claude CLI integration does exist — `dsh-subagent-claude-code` runs the real CLI through the official Agent SDK under the shared subprocess owner — but it registers a subagent provider rather than an `LlmAdapter`, and it uses the SDK this note places outside the design. Whether Candy writes a direct `--output-format` parser or reuses that SDK path is an open decision this bullet settles first; only then can a strict parser, its version probe, and its fixtures be written.
