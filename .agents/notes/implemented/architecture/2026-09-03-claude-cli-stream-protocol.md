...
The R2 adapter can be written against a protocol that is tested, so its own tests can be about process lifetime, cancellation, and cleanup rather than about parsing. Two costs are accepted. The package is pinned to one CLI version: the open frame union absorbs additions, but a renamed field this package acts on would surface as translation silently seeing no content, which only re-recording the fixtures would catch. And only the invocation's cost total is carried: `total_cost_usd` reaches `TokenUsage.costMicroUsd`, while the per-model breakdown beside it is dropped, so a recorded two-token request that also billed a `claude-haiku-4-5` call reports one figure covering both.
