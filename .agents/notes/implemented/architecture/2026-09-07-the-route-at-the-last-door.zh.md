...
`dsh-tenant-route-policy` 是 Candy 专属消费方。对于携带 session 的请求，它通过 `RunScheduler.tenantOf` 解析租户，并要求该租户配置的白名单中存在区分大小写的精确 provider/model 组合。配置中缺少的受管租户会被拒绝，空列表同样如此。拒绝发生在适配器选择之前，并表现为带 `TENANT_ROUTE_NOT_ALLOWED` 的终止流错误。
...
受管租户采用封闭配置，因此启用插件时必须为每个应当运行的租户提供条目。拒绝会在返回前等待 `RunScheduler.recordRouteRefusal()`，留下按租户归档的 `refused` 记录，其中 `action: 'route'`，outcome 为策略 code。配置仍是部署状态，而不是控制平面状态。
